...
public class UpdateAuditTriggerIntegrationTest {
    ...
    static void beforeAll() throws SQLException, LiquibaseException {
        ...
        liquibase = new Liquibase("db/changelog/update-audit-trigger-tests.xml", new ClassLoaderResourceAccessor(), database);
        liquibase.update("");
    }

    @BeforeEach
    public void beforeEach() throws Exception {
        deleteAllRows("test_tbl");
        deleteAllRows("test_tbl_audit");
    }
    ...
    @Test
    void auditTriggerHandlesAddedColumn() throws Exception {

        String sql = "INSERT INTO test_tbl (text_col, int_col, bool_col) VALUES (?, ?, ?)";

        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            preparedStatement.setString(1, "some-text");
        ...
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("select * from test_tbl_audit")) {
            assertTrue(rs.next());
            assertEquals("some-text", rs.getString("text_col"));
            assertEquals(27, rs.getInt("int_col"));
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            preparedStatement.setString(1, "some-more-text");
        ...
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("select * from test_tbl_audit")) {
            assertTrue(rs.next());
            assertEquals("some-more-text", rs.getString("text_col"));
            assertEquals(29, rs.getInt("int_col"));
